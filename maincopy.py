...
def analyze_student_detailed(agent, student_id):
    ...
    print("\n--- 短期干预计划 (1-2周) ---")
    print("目标:")
    for i, goal in enumerate(intervention_plan["short_term"]["目标"], 1):
        print(f"  {i}. {goal}")
    
    print("\n具体行动:")
    for i, action in enumerate(intervention_plan["short_term"]["具体行动"], 1):
        print(f"  {i}. {action}")
    
    # 中期计划 (简要)
    print("\n--- 中期干预计划 (1-2个月) ---")
    print("目标:")
    for i, goal in enumerate(intervention_plan["medium_term"]["目标"], 1):
        print(f"  {i}. {goal}")
    
    # 长期计划 (简要)
    print("\n--- 长期干预计划 (一学期或更长) ---")
    print("目标:")
    for i, goal in enumerate(intervention_plan["long_term"]["目标"], 1):
        print(f"  {i}. {goal}")
